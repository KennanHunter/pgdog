...
// All statements below use `simple_query` / `batch_execute` with the shard key
// inlined. tokio-postgres's `.query`/`.execute` use the extended protocol
// (Parse/Bind/Execute), which sends the shard column as a bound parameter —
// PgDog can't extract the value until Bind, so it broadcasts to every shard.
// That's fine for a plain query, but a broadcast inside a pinned transaction
// fails with "cannot switch shards in a direct-to-shard transaction".
// Simple protocol lets PgDog parse the shard key from the query text.
...
async fn transaction_once(client: &Client, key_space: i64) -> Result<(), tokio_postgres::Error> {
    let id = random_id(key_space);
    let value = random_value();
    // Each statement is a separate simple_query so PgDog can parse and route
    // the sharded ones by their `id` literal. A multi-statement batch would be
    // broadcast to every shard, which leaves stray backends `idle in transaction`
    // on the shards the client never comes back to commit.
    ...
    client
        ...
        .await?;
    // Keep the transaction open a moment. During a transaction PgDog pins the
    // client to the server the transaction started on.
    let hold_ms = 15 + rand::thread_rng().gen_range(0..25);
    sleep(Duration::from_millis(hold_ms)).await;
    client
        .simple_query(&format!("SELECT value FROM kv WHERE id = {id}"))
